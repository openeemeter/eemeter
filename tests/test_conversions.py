...
def test_kwh_to_BTU():
    assert abs(kwh_to_BTU(-1) - -3.41214163e3) < EPSILON
    assert abs(kwh_to_BTU(0) - 0) < EPSILON
    assert abs(kwh_to_BTU(1) - 3.41214163e3) < EPSILON

def test_BTU_to_kwh():
    assert abs(BTU_to_kwh(-1) - -2.9307107e-4) < EPSILON
    assert abs(BTU_to_kwh(0) - 0) < EPSILON
    assert abs(BTU_to_kwh(1) - 2.9307107e-4) < EPSILON

def test_therm_to_BTU():
    assert abs(therm_to_BTU(-1) - -9.9976129e4) < EPSILON
    assert abs(therm_to_BTU(0) - 0) < EPSILON
    assert abs(therm_to_BTU(1) - 9.9976129e4) < EPSILON

def test_BTU_to_therm():
    assert abs(BTU_to_therm(-1) - -1.00023877e-5) < EPSILON
    assert abs(BTU_to_therm(0) - 0) < EPSILON
    assert abs(BTU_to_therm(1) - 1.00023877e-5) < EPSILON

def test_therm_to_kwh():
    assert abs(therm_to_kwh(-1) - -29.3001111) < EPSILON
    assert abs(therm_to_kwh(0) - 0) < EPSILON
    assert abs(therm_to_kwh(1) - 29.3001111) < EPSILON

def test_kwh_to_therm():
    assert abs(kwh_to_therm(-1) - -0.0341295634) < EPSILON
