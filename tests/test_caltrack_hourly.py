...
#     model_results = caltrack_hourly_method(
#             baseline_data, formula, preprocessors)
#     e2e_warnings.extend(model_results.warnings)
#     assert isinstance(model_results, ModelResults)
#     assert model_results.status == 'SUCCESS'
#     assert model_results.method_name == 'caltrack_hourly_method'
#     assert model_results.settings == {'preprocessors': preprocessors,
#                                   'formula': formula}
#     assert isinstance(model_results.model, HourlyModel)
#     assert len(model_results.model.model_object) == 12
#     assert isinstance(list(model_results.model.model_object.values())[0],
#                       statsmodels.regression.linear_model.WLS)
#     assert isinstance(model_results.model.model_params, pd.DataFrame)
#     assert model_results.model.model_params.shape == (12, 1 + 168 + 7)
#     assert model_results.model.__repr__() == \
...
#     model_prediction = model_results.model.predict(baseline_data)
...
#     assert model_results.method_name == 'caltrack_hourly_method'
#     assert model_results.status == 'NOT ATTEMPTED'
#     assert len(model_results.warnings) == 1
#     warning = model_results.warnings[0]
#     assert warning.qualified_name == (
...
#             data, formula='meter_value ~ hour_of_week')
#     assert model_results.method_name == 'caltrack_hourly_method'
#     assert model_results.status == 'FAILED'
#     assert len(model_results.warnings) == 1
#     warning = model_results.warnings[-1]
#     assert warning.qualified_name == (
...
#         'bin_6': [22, 44, 66],
#     })
#     model_results = caltrack_hourly_method(data)
#     assert model_results.method_name == 'caltrack_hourly_method'
#     assert model_results.status == 'SUCCESS'
#     assert len(model_results.warnings) == 1
#     warning = model_results.warnings[-1]
