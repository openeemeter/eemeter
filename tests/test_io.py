...
def test_meter_data_from_csv_custom_columns(sample_metadata):
    with TemporaryFile() as f:
        ...
        f.seek(0)
        meter_data = meter_data_from_csv(
            f, start_col='start_custom', value_col='kWh')
...
def test_meter_data_from_json_orient_list(sample_metadata):
    data = [
        ['2017-01-01T00:00:00Z', 11],
        ['2017-01-02T00:00:00Z', 10],
    ]
...
def test_meter_data_to_csv(sample_metadata):
    df = pd.DataFrame(
        {'value': [5]},
        index=pd.DatetimeIndex(['2017-01-01T00:00:00Z'])
    )
