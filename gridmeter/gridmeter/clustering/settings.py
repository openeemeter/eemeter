...
class Settings(pydantic.BaseModel):
    ...
    score_choice: _const.ScoreChoice = pydantic.Field(default=_const.ScoreChoice.VARIANCE_RATIO, validate_default=True)
    """scoring choice to determine best cluster composition to use"""

    dist_metric: _const.DistanceMetric = pydantic.Field(default=_const.DistanceMetric.EUCLIDEAN, validate_default=True)
    ...
    agg_type: _const.AggType = pydantic.Field(default=_const.AggType.MEDIAN, validate_default=True)

    max_non_outlier_cluster_count: int = pydantic.Field(default=200, validate_default=True)
