...
def master_function(
    df_t: pd.DataFrame, df_c: pd.DataFrame, settings: _settings.Settings
):
    """ """

    matcher = _cluster.ClusterResult.from_comparison_pool_loadshapes_and_settings(
        df_cp_ls=df_c, s=settings
    )
    treatment_df = matcher.get_match_treatment_to_cluster_df(
        treatment_loadshape_df=df_t
    )
    _ = 1
