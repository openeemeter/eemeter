...
def weekly_load_shape(daily_load_shape, noise_sigma=0.05):
    ...
    return pd.concat(
        [
            pd.DataFrame(
                {
                    "t": f"{i}." + daily_load_shape.t.astype(str),
                    "value": daily_load_shape.value * rand(),
                }
            )
            for i in range(7)
        ]
...
class DummyMeter:
    """
    Compute dummy data for one meter.  A very basic 24-hr load
    ...
    def __init__(
        ...
        self.load_shape_winter = daily_load_shape(
            self.winter_base, self.winter_peak, t_up=9, t_down=20
        )
        self.load_shape_summer = daily_load_shape(
            self.summer_base, self.summer_peak, t_up=9, t_down=20
        )
        self.load_shape_shoulder = daily_load_shape(
            self.shoulder_base, self.shoulder_peak, t_up=9, t_down=20
        )
    ...
    def features_seasonal_168(self):
        winter_week = weekly_load_shape(
            self.load_shape_winter, noise_sigma=self.noise_sigma
        ).assign(season="winter")
        shoulder_week = weekly_load_shape(
            self.load_shape_shoulder, noise_sigma=self.noise_sigma
        ).assign(season="shoulder")
        summer_week = weekly_load_shape(
            self.load_shape_summer, noise_sigma=self.noise_sigma
    ...
    def features(self):
        ...
        return pd.DataFrame(
            {
                "winter_usage": winter_usage,
                "summer_usage": summer_usage,
...
class DummyPopulation:
    """
    Compute dummy data for a population of meters.  See `DummyMeter`.
        ...
        self.df_params = pd.DataFrame(
            {
                ...
                "winter_base": np.random.lognormal(
                    mean=winter_base_mean, sigma=winter_base_sigma, size=n_meters
                ),
                "winter_peak": np.random.lognormal(
                    mean=winter_peak_mean, sigma=winter_peak_sigma, size=n_meters
                ),
                "summer_base": np.random.lognormal(
                    mean=summer_base_mean, sigma=summer_base_sigma, size=n_meters
                ),
                "summer_peak": np.random.lognormal(
                    mean=summer_peak_mean, sigma=summer_peak_sigma, size=n_meters
                ),
                "shoulder_base": np.random.lognormal(
                    mean=shoulder_base_mean, sigma=shoulder_base_sigma, size=n_meters
                ),
                "shoulder_peak": np.random.lognormal(
                    mean=shoulder_peak_mean, sigma=shoulder_peak_sigma, size=n_meters
                ),
            }
        )
...
class DummyTreatmentPoolPopulation:
    """
    Compute dummy data for a treatment population and
    comparison pool.  See `DummyMeter` and `DummyPopulation`.
    Data will be cached on disk in the folder specified in `cache_folder`.
    ...
    def __init__(
        ...
        n_treatment=100,
        n_pool=1000,
        winter_base_mean=0.1,
        winter_base_sigma=0.1,
        winter_peak_mean=1,
        winter_peak_sigma=0.1,
        summer_base_mean=0.1,
        summer_base_sigma=0.1,
        summer_peak_mean=0.8,
        summer_peak_sigma=0.1,
        shoulder_base_mean=0.1,
        shoulder_base_sigma=0.1,
        shoulder_peak_mean=0.9,
        shoulder_peak_sigma=0.1,
        treatment_filter_function=lambda df: df[
            df.annual_usage > df.annual_usage.quantile(0.2)
        ],
        cache_folder=".cache",
    ):
        logging.info(f"Caching objects to {cache_folder}")

        self.population = DummyPopulation(
            n_meters=n_treatment + n_pool,
            id_prefix="meter",
        ...
        df_features = (
            self.population.features()
            .reset_index()
            .rename(columns={"index": "meter_id"})
        )
        self.treatment_meters = treatment_filter_function(df_features)[
            "meter_id"
        ].sample(n_treatment)
        self.pool_meters = df_features.meter_id[
            ~df_features.meter_id.isin(self.treatment_meters)
        ]
