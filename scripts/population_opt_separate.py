...
for subsample in subsamples:
    for solar_meter in solar_meters:
        save_path = "/app/.recurve_cache/mce_3_yr_precovid/Hyperparameter_opt/separateV1"
        save_path = save_path + f"/subsample_{subsample}_solar_meter_{solar_meter}.pkl"
        if os.path.exists(save_path):
            print(f"subsample: {subsample}, solar_meter: {solar_meter} already exists")
        else:
            kwargs['subsamples'] = [subsample]
            kwargs['solar_meters'] = [solar_meter]
            ...
            print(f"subsample: {subsample}, solar_meter: {solar_meter}, loss: {loss}, x_opt: {x_opt}, itercount: {itercount}, evalcount: {evalcount}, fast_evalcount: {fast_evalcount}")
            #save the results for each subsample and solar_meter
            results = {
                'subsample': subsample,
