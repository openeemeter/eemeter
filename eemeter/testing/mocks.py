...
class MockWeatherClient(object):
    ...
    def _fake_temps(self, n, num_years=1):
    ...
    def load_gsod_data_eeweather(self, station, start, end):
        dates = pd.date_range("{}-01-01 00:00".format(start.year),
                              "{}-12-31 00:00".format(end.year),
                              freq='D', tz=pytz.UTC)
        num_years = end.year - start.year + 1
        temps = self._fake_temps(dates.shape[0], num_years)
        return pd.Series(temps, index=dates, dtype=float)
