...
from .api import EEMeterWarning, ModelResults, HourlyModel, ModelPrediction
        ...
        warnings.append(
            EEMeterWarning(
                ...
                data={
                    ...
                    "missing_months": [
                        month for month in range(1, 13) if month not in captured_months
                    ],
                },
            )
        )
            ...
            warnings.append(
                EEMeterWarning(
                    ...
                    description=(
                        ...
                        "Month {}: Coverage: {}".format(
                            month, row.hourly_coverage.values[0]
                        )
                    ),
                    data={
                        "model_id": model_id,
...
def segment_timeseries(data, segment_type):
    ...
    valid_segment_types = ["one_month", "three_month", "three_month_weighted", "single"]
    ...
    for column in ["meter_value", "temperature_mean"]:
        if column not in data.columns:
            raise ValueError("Data does not include columns: {}".format(column))

    for column in ["model_id", "weight"]:
        if column in data.columns:
            raise ValueError("Data already contains column: {}".format(column))

    if segment_type == "one_month":
        data_segmented = data.copy()
        data_segmented["weight"] = 1
        data_segmented["model_id"] = [tuple([x]) for x in data_segmented.index.month]
        ...
        months.loc[:, "baseline"] = months.model_id.map(lambda x: shoulder_months(x[0]))
        ...
        for i, month in months.iterrows():
            ...
            warnings.extend(
                get_hourly_coverage_warning(this_df, month.baseline, month.model_id)
            )
        ...
        data_segmented = data.copy()
        data_segmented["weight"] = 1
        data_segmented["model_id"] = [tuple(range(1, 13))] * len(data_segmented.index)
...
def get_feature_hour_of_week(data):
    ...
    feature_hour_of_week["hour_of_week"] = feature_hour_of_week["hour_of_week"].astype(
        "category"
    )

    hour_warnings.extend(
        get_missing_hours_of_week_warning(feature_hour_of_week.hour_of_week.unique())
    )
...
def get_single_feature_occupancy(data, threshold):
    ...
    try:
        ...
        warnings.extend(
            get_fit_failed_model_warning(data.model_id[0], "occupancy_model")
        )
    ...
    occupancy_lookup = pd.DataFrame(
        {
            "occupancy": model_data.groupby(["hour_of_week"]).apply(
                lambda x: is_high_usage(x, threshold)
            )
        }
    ).reset_index()
...
def get_feature_occupancy(
    ...
    if mode == "fit":
        ...
        for model_id in unique_models:
            ...
            this_model, this_occupancy_lookup, this_warnings = get_single_feature_occupancy(
                this_data, threshold
            )
            this_occupancy_lookup["model_id"] = [model_id] * len(
                this_occupancy_lookup.index
            )
    ...
    feature_hour_of_week, parameters, warnings = get_feature_hour_of_week(data_verified)

    feature_occupancy = (
        data_verified.reset_index()
        .merge(
            feature_hour_of_week,
            left_on=["start", "model_id"],
            right_on=["start", "model_id"],
...
def assign_temperature_bins(data, bin_endpoints):
    ...
    for ibin in range(ibin_final):
        ...
        if ibin == 0:
            ...
            tdata.loc[gt_bin, bin_name] = bin_endpoints[ibin + 1] - bin_endpoints[ibin]
            tdata.loc[lt_bin, bin_name] = 0
    return tdata.drop("temperature_mean", axis=1)


def validate_temperature_bins(data, default_bins, min_temperature_count=20):
    ...
    for i in range(1, len(bin_endpoints_valid) - 1):  # not pythonic
        ...
        temperature_data.bin = temperature_data.bin.cat.set_categories(bins_default)

        temperature_summary = (
            temperature_data.groupby("bin").count().sort_index().reset_index()
        )
        ...
        first_bin_below_threshold = (
            temperature_summary.temperature_mean.iloc[0] < min_temperature_count
        )
        ...
        last_bin_below_threshold = (
            temperature_summary.temperature_mean.iloc[-1] < min_temperature_count
        )
    ...
    bin_endpoints_valid = [
        x for x in bin_endpoints_valid if x not in [-1000000, 1000000]
    ]
...
def get_feature_binned_temperatures(
    ...
    if mode == "fit":
        ...
        for model_id in unique_models:
            this_data = data_verified.loc[data_verified.model_id == model_id]
            this_summary, this_valid_bins = validate_temperature_bins(
                this_data, default_bins, **kwargs
            )
            this_summary["model_id"] = [model_id] * len(this_summary.index)
            temperature_bins = temperature_bins.append(
                pd.DataFrame({"model_id": [model_id], "bins": [this_valid_bins]}),
                sort=False,
            )
            temperature_summary = temperature_summary.append(this_summary, sort=False)

    else:  # mode == 'predict'
        temperature_bins = kwargs["temperature_bins"]

        missing_columns = any(
            column not in temperature_bins.columns for column in ["bins", "model_id"]
        )
    ...
    for model_id in unique_models:
        this_data = data_verified.loc[data_verified.model_id == model_id]
        this_valid_bins = temperature_bins.loc[
            temperature_bins.model_id == model_id
        ].bins[0]
        this_feature = assign_temperature_bins(this_data, this_valid_bins)
        this_feature["model_id"] = [model_id] * len(this_feature.index)
        feature_binned_temperatures = feature_binned_temperatures.append(
            this_feature, sort=False
        )
    ...
    return (feature_binned_temperatures, temperature_parameters, temperature_warnings)
...
def handle_unsegmented_timeseries(data):
    ...
    if "model_id" not in data_verified.columns:
        ...
        data_verified["model_id"] = [tuple(range(1, 13))] * len(
            data_verified.index
        )  # odd model id

    if "weight" not in data_verified.columns:
        warnings.extend(get_missing_weight_column_warning(data_verified.columns))
...
def get_design_matrix(data, functions):
    ...
    design_matrix_warnings.extend(get_invalid_function_dict_warning(feature_functions))
    ...
    for name, function in feature_functions.items():
        if function["function"].__name__ == "segment_timeseries":
            try:
                data_verified, warnings = segment_timeseries(data, **function["kwargs"])
    ...
    for name, function in feature_functions.items():
        try:
            this_feature, this_parameters, this_warnings = function["function"](
                data_verified, **function["kwargs"]
            )
        except TypeError:
            design_matrix_warnings.extend(
                get_design_matrix_wrong_kwargs_warning(function)
        ...
        if any(this_feature.sort_index().index != data_verified.sort_index().index):
            design_matrix_warnings.extend(
                get_design_matrix_unmatched_index_warning(function)
            )
            return pd.DataFrame(), dict(), design_matrix_warnings
        ...
        design_matrix = design_matrix.merge(
            this_feature, left_on=["start", "model_id"], right_on=["start", "model_id"]
        )
        preprocessors_fit.update(
            {name: {"function": function["function"], "kwargs": this_parameters}}
        )
...
def get_missing_features_warning(formula, columns):
    warning = [
        EEMeterWarning(
            qualified_name=("eemeter.caltrack_hourly.missing_features"),
            description=("Data is missing features specified in formula."),
            data={"formula": formula, "dataframe_columns": columns},
        )
    ]
    return warning
...
def get_single_model(data, formula):
    ...
    try:
        with ws.catch_warnings(record=True) as sm_warnings:
            model_consumption = smf.wls(formula=formula, data=data, weights=data.weight)
            if len(sm_warnings) > 0:
                raise RuntimeError([w.message for w in sm_warnings])
    except Exception as e:
        warnings.extend(
            get_fit_failed_model_warning(data.model_id[0], "consumption_model")
        )
        return None, pd.DataFrame(), warnings
...
def caltrack_hourly_method(data, formula=None, preprocessors=None):
    ...
    if preprocessors is None:
        ...
        design_matrix, preprocessors_fit, design_matrix_warnings = get_design_matrix(
            data, preprocessors
        )
        if len(design_matrix_warnings) > 0:
            return ModelResults(
    ...
    for model_id in unique_models:
        ...
        this_model, this_parameters, this_warnings = get_single_model(
            this_data, formula
        )
        this_parameters["model_id"] = [model_id] * len(this_parameters.index)
    ...
    return ModelResults(
        ...
        candidates=None,
        warnings=method_warnings,
        settings={"preprocessors": preprocessors, "formula": formula},
    )
...
def caltrack_hourly_predict(
    ...
    if preprocessors_fit is None:
        ...
        design_matrix, _fit, design_matrix_warnings = get_design_matrix(
            data_verified, preprocessors_fit
        )
        if len(design_matrix_warnings) > 0:
    ...
    if any(term not in design_matrix.columns for term in term_list):
        predict_warnings.extend(
            get_missing_features_warning(formula, design_matrix.columns.tolist())
        )
        return ModelPrediction(
            result=pd.DataFrame(),
    ...
    design_matrix_granular = dmatrix(
        formula.split("~")[1], design_matrix, return_type="dataframe"
    )
    ...
    for model_id in unique_models:
        this_parameters = model_params.loc[model_params.model_id == model_id].drop(
            "model_id", axis=1
        )
        this_data = design_matrix_granular.loc[
            design_matrix_granular.model_id == model_id
    ...
    return ModelPrediction(
        result=results, design_matrix=design_matrix_granular, warnings=predict_warnings
    )
