...
def obj_fcn_decorator(
    ...
    def elastic_net_penalty(X, T_sorted, obs_sorted, weight_sorted):
        ...
        if (len(idx_bp) == 2) and (len(idx_hdd) >= N_min) and (len(idx_cdd) >= N_min):
            ...
            T_stdev = np.array([stdev(T_sorted[idx_hdd], weights=weight_sorted[idx_hdd]),
                                stdev(T_sorted[idx_cdd], weights=weight_sorted[idx_cdd])])
            obs_stdev = np.array([stdev(obs_sorted[idx_hdd], weights=weight_sorted[idx_hdd]),
        ...
        if len(idx_k) > 0: # reducing X_enet size allows for more smoothing
            ...
            X_enet[idx_k] *= X_enet[idx_beta]/2 # uncertain what to divide by, this seems to work well
    ...
    def obj_fcn(X, grad=[], optimize_flag=True):
        ...
        for C in C_list:
            if C is None:
                weight_sorted, c, a = weight_fcn(*X, T_sorted, resid_sorted, sigma, quantile, alpha, min_weight)
                ...
                try: # I know it's ugly, but it won't break?
                    ...
                    if isinstance(C, float):
                        ...
                        c = rolling_C(T_sorted, resid_sorted, mu, sigma, 0.25, window, step)
                
                    weight_sorted, a = adaptive_weights(resid_sorted, mu=mu, c=c, alpha=alpha, min_weight=min_weight)
