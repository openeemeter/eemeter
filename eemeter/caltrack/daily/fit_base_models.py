...
from eemeter.caltrack.daily.base_models.hdd_tidd_cdd_smooth import fit_hdd_tidd_cdd_smooth
...
def _get_opt_options(settings):
    ...
    opt_options = {"global": {"algorithm": settings.algorithm_choice, 
...
def fit_initial_models_from_full_model(df_meter, settings, print_res=False):
    ...
    if (settings.full_model == FullModelSelection.HDD_TIDD_CDD) and settings.smoothed_model:
    ...
    elif (settings.full_model == FullModelSelection.C_HDD_TIDD) and settings.smoothed_model:
    ...
    if print_res:
        ...
        print(f"{model_res.model_name:<30s} {criterion:<8.3g} {model_res.alpha:<8.2f} {model_res.time_elapsed:>8.2f} ms")
