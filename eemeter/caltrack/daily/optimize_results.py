...
def reduce_model(hdd_bp, hdd_beta, pct_hdd_k, cdd_bp, cdd_beta, pct_cdd_k, intercept, T_min, T_max, T_min_seg, T_max_seg, model_key):
    ...
    if (hdd_beta != 0) and (cdd_beta == 0) and (pct_hdd_k != 0):
        ...
        if model_key == "hdd_tidd_cdd_smooth":
            [hdd_bp, hdd_k, cdd_bp, cdd_k] = get_k([hdd_bp, pct_hdd_k, cdd_bp, pct_cdd_k], T_min_seg, T_max_seg)
...
class OptimizedResult:
    def __init__(self, x, bnds, coef_id, loss_alpha, C, T, model, weight, resid, jac,
                 mean_loss, TSS, success, message, nfev, time_elapsed, settings
                 ):
        ...
        if jac is not None: # for future uncertainty calculations
            ...
            self.x_unc = np.sqrt(np.diag(self.cov))*unc_factor(self.DoF + 1, interval="PI", alpha=unc_alpha)
    ...
    def _prediction_uncertainty(self): # based on std
        ...
        acorr = acf(self.resid, lag_n=1, moving_mean_std=False) # only check 1 day of lag
    ...
    def _set_model_key(self):
        if self.coef_id == ["hdd_bp", "hdd_beta", "hdd_k", "cdd_bp", "cdd_beta", "cdd_k", "intercept"]:
    ...
    def _refine_model(self):
        ...
        x = get_full_model_x(self.model_key, self.x, self.T_min, self.T_max, self.T_min_seg, self.T_max_seg)

        # reduce model
        self.coef_id, self.x = reduce_model(*x, self.T_min, self.T_max, self.T_min_seg, self.T_max_seg, self.model_key)
