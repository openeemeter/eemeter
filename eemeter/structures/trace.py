...
class EnergyTrace(object):
    ...
    ----------
    interpretation : str
        The way this energy time series in the `data` attribute should be
        interpreted. The complete list of supported options is as follows:

        - `ELECTRICITY_CONSUMPTION_SUPPLIED`: Represents the amount of
        ...
        - `ELECTRICITY_CONSUMPTION_TOTAL`: Represents the amount of
          ...
          electricity meter, to `ELECTRICITY_CONSUMPTION_SUPPLIED` -
          `ELECTRICITY_ON_SITE_GENERATION_CONSUMED`.
        - `ELECTRICITY_CONSUMPTION_NET`: Represents the amount of
        ...
        - `ELECTRICITY_ON_SITE_GENERATION_TOTAL`: Represents the amount of
          ...
          meter, to `ELECTRICITY_ON_SITE_GENERATION_CONSUMED` +
          `ELECTRICITY_ON_SITE_GENERATION_UNCONSUMED`.
        - `ELECTRICITY_ON_SITE_GENERATION_CONSUMED`: Represents the amount of
          locally generated electrical energy consumed on-site, such as energy
          generated by solar photovoltaic panels.
        - `ELECTRICITY_ON_SITE_GENERATION_UNCONSUMED`: Represents the amount of
          excess locally generated energy, which instead of being consumed
          on-site, is fed back into the grid or sold back a utility.
        - `NATURAL_GAS_CONSUMPTION_SUPPLIED`: Represents the amount of energy
          ...
          `NATURAL_GAS_CONSUMPTION_TOTAL` and `NATURAL_GAS_CONSUMPTION_NET`
          would be equivalent for natural gas,
          `NATURAL_GAS_CONSUMPTION_SUPPLIED` is prefered for its greater
          specificity.
    ...
    data : pandas.DataFrame, default None
        ...
        always be `NaN`, since is purpose is only to cap the end of the last
        ...
        such as those specified in pandas using the `freq` attribute.
        ...
        If `serializer` instance is provided, this should instead be records in
        the format expected by the serializer.
    unit : str
        The name of the unit in which the energy time series is given. These
        names are normalized to either `'KWH'` or `'THERM'` as follows:
