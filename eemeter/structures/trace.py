...
class EnergyTrace(object):
    ...
    interpretation : str
        ...
        - :code:`ELECTRICITY_ON_SITE_GENERATION_TOTAL`: Represents the amount of
          locally generated electrical energy consumed on-site plus the amount
          of locally generated elecrical energy returned to the grid, as
          metered at a single usage point. Equivalent, for a single electricity
          meter, to :code:`ELECTRICITY_ON_SITE_GENERATION_CONSUMED` +
          :code:`ELECTRICITY_ON_SITE_GENERATION_UNCONSUMED`.
        - :code:`ELECTRICITY_ON_SITE_GENERATION_CONSUMED`: Represents the amount of
          locally generated electrical energy consumed on-site, such as energy
          generated by solar photovoltaic panels.
        - :code:`ELECTRICITY_ON_SITE_GENERATION_UNCONSUMED`: Represents the amount of
          excess locally generated energy, which instead of being consumed
          on-site, is fed back into the grid or sold back a utility.
        - :code:`NATURAL_GAS_CONSUMPTION_SUPPLIED`: Represents the amount of energy
          ...
          :code:`NATURAL_GAS_CONSUMPTION_TOTAL` and :code:`NATURAL_GAS_CONSUMPTION_NET`
          would be equivalent for natural gas,
          :code:`NATURAL_GAS_CONSUMPTION_SUPPLIED` is prefered for its greater
          specificity.
    ...
    data : pandas.DataFrame, default None
        ...
        always be :code:`NaN`, since is purpose is only to cap the end of the last
        period, and not to represent a time period over which energy was
        ...
        - :code:`estimated`: Whether or not the value was estimated. Particularly
          relevant for monthly billing data.

        If :code:`serializer` instance is provided, this should instead be records in
        the format expected by the serializer.
    unit : str
        The name of the unit in which the energy time series is given. These
        names are normalized to either :code:`'KWH'` or :code:`'THERM'` as follows:

        - :code:`'kwh'` becomes :code:`'KWH'` with no unit conversion multiplier.
        - :code:`'kWh'` becomes :code:`'KWH'` with no unit conversion multiplier.
        - :code:`'KWH'` becomes :code:`'KWH'` with no unit conversion multiplier.
        - :code:`'therm'` becomes :code:`'THERM'` with no unit conversion multiplier.
        - :code:`'therms'` becomes :code:`'THERM'` with no unit conversion multiplier.
        - :code:`'thm'` becomes :code:`'THERM'` with no unit conversion multiplier.
        - :code:`'THERM'` becomes :code:`'THERM'` with no unit conversion multiplier.
        - :code:`'THERMS'` becomes :code:`'THERM'` with no unit conversion multiplier.
        - :code:`'THM'` becomes :code:`'THERM'` with no unit conversion multiplier.
        - :code:`'wh'` becomes :code:`'KWH'` with a unit conversion multiplier of :code:`0.001`.
        - :code:`'Wh'` becomes :code:`'KWH'` with a unit conversion multiplier of :code:`0.001`.
        - :code:`'WH'` becomes :code:`'KWH'` with a unit conversion multiplier of :code:`0.001`.
