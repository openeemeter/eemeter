...
def merge_features(features, keep_partial_nan_rows=False):
    ...
    df = pd.concat([
        _to_frame_if_needed(feature) for feature in features
    ], axis=1)
...
def _estimate_hour_of_week_occupancy(model_data, threshold):
    ...
    return model_data_with_residuals \
        ...
        .rename('occupancy') \
        .reindex(pd.CategoricalIndex(range(1, 169)))  # guarantee an index value for all hours
...
def _fit_temperature_bins(
    ...
    def _compute_temp_summary(bins):
        ...
        temp_bins = pd.cut(temperature_data, bins=bins) \
            .cat.set_categories(bin_intervals)
        return pd.DataFrame({
            'temp': temperature_data,
            'bin': temp_bins,
        }).groupby('bin')['temp'].count().rename('count').sort_index()
...
def fit_temperature_bins(
    data, segmentation=None, default_bins=[30, 45, 55, 65, 75, 90],
    ...
    for segment_name, segmented_data in segmented_datasets:
        segmented_bins[segment_name] = _fit_temperature_bins(
            segmented_data.temperature_mean, default_bins,
