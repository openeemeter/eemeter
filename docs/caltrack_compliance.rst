...
- **2.2.1.2**: :any:`eemeter.caltrack_sufficiency_criteria` must set ``min_fraction_daily_coverage=0.9`` *Not Compliant (If this is used for reporting data that does not span exactly 365 days, then the daily coverage must be refactored to be evaluated for each year of data rather than overall.*
